import React from "react";

export const FilterIcon = () => (
  <svg className="svg-next-icon svg-next-icon-size-14" width="14" height="14">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 459 459">
      <g>
        <path d="M178.5,382.5h102v-51h-102V382.5z M0,76.5v51h459v-51H0z M76.5,255h306v-51h-306V255z"></path>
      </g>
    </svg>
  </svg>
);

export const SearchIcon = () => (
  <svg className="svg-next-icon svg-next-icon-size-18" width="18" height="18">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
      <path d="M8 12a4 4 0 1 1 0-8 4 4 0 0 1 0 8m9.707 4.293l-4.82-4.82A5.968 5.968 0 0 0 14 8 6 6 0 0 0 2 8a6 6 0 0 0 6 6 5.968 5.968 0 0 0 3.473-1.113l4.82 4.82a.997.997 0 0 0 1.414 0 .999.999 0 0 0 0-1.414"></path>
    </svg>
  </svg>
);

export const AddIcon = () => (
  <svg className="svg-next-icon svg-next-icon-size-16" width="16" height="16">
    <svg viewBox="0 0 14 14" xmlns="http://www.w3.org/2000/svg">
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M6.99992 0.333374C3.31992 0.333374 0.333252 3.32004 0.333252 7.00004C0.333252 10.68 3.31992 13.6667 6.99992 13.6667C10.6799 13.6667 13.6666 10.68 13.6666 7.00004C13.6666 3.32004 10.6799 0.333374 6.99992 0.333374ZM6.33325 3.66671V6.33337H3.66658V7.66671H6.33325V10.3334H7.66658V7.66671H10.3333V6.33337H7.66658V3.66671H6.33325ZM1.66659 7.00004C1.66659 9.94004 4.05992 12.3334 6.99992 12.3334C9.93992 12.3334 12.3333 9.94004 12.3333 7.00004C12.3333 4.06004 9.93992 1.66671 6.99992 1.66671C4.05992 1.66671 1.66659 4.06004 1.66659 7.00004Z"
      ></path>
    </svg>
  </svg>
);

export const CloseIcon = () => (
  <span className="d-inline-block">
    <svg
      className="svg-next-icon cursor-pointer svg-next-icon-size-16"
      width="16"
      height="16"
    >
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" version="1.1">
        <path d="M17,15.584 L13.4,11.984 L17,8.384 L15.584,6.968 L11.984,10.568 L8.384,6.968 L6.968,8.384 L10.568,11.984 L6.968,15.584 L8.384,17 L11.984,13.4 L15.584,17 L17,15.584 Z M11.984,2 C13.8080091,2 15.4959922,2.45599544 17.048,3.368 C18.5520075,4.23200432 19.7359957,5.41599248 20.6,6.92 C21.5120046,8.47200776 21.968,10.1599909 21.968,11.984 C21.968,13.8080091 21.5120046,15.4959922 20.6,17.048 C19.7359957,18.5520075 18.5520075,19.7359957 17.048,20.6 C15.4959922,21.5120046 13.8080091,21.968 11.984,21.968 C10.1599909,21.968 8.47200776,21.5120046 6.92,20.6 C5.41599248,19.7199956 4.23200432,18.5280075 3.368,17.024 C2.45599544,15.4719922 2,13.792009 2,11.984 C2,10.175991 2.45599544,8.49600776 3.368,6.944 C4.2480044,5.43999248 5.43999248,4.2480044 6.944,3.368 C8.49600776,2.45599544 10.175991,2 11.984,2 Z"></path>
      </svg>
    </svg>
  </span>
);

export const Box = () => (
  <svg
    className="svg-next-icon mb-5"
    width="100"
    height="100"
    style={{ color: "#c3cfd8", width: "100px", height: "100px" }}
  >
    <svg
      id="Layer_1"
      data-name="Layer 1"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 208 230"
    >
      <defs>
        <linearGradient
          id="linear-gradient"
          x1="133.69"
          y1="120.61"
          x2="73.06"
          y2="91.22"
          gradientTransform="matrix(1, 0, 0, -1, 0, 231.28)"
          gradientUnits="userSpaceOnUse"
        >
          <stop offset="0" stopColor="#95abc2"></stop>
          <stop offset="1" stopColor="#fff"></stop>
        </linearGradient>
        <mask
          id="mask"
          x="134.5"
          y="14.55"
          width="17.97"
          height="17.21"
          maskUnits="userSpaceOnUse"
        >
          <g id="mask0">
            <path
              d="M137,24a8.1,8.1,0,0,1,8.1-8.1h6.3v6.3a8.1,8.1,0,0,1-8.1,8.1H137Z"
              fill="#fff"
            ></path>
          </g>
        </mask>
      </defs>
      <title>Artwork</title>
      <path d="M94.2,85.7l85.9,23.6v82.6l-85.9-37Z" fill="#8e99a3"></path>
      <path d="M94.3,85.7,24.1,105.3v86.8l70.1-37.6Z" fill="#a0aab5"></path>
      <path
        d="M139.4,185.9c-19.4-5.7-53.4-5.2-66.2-38.3-11.7-30.4,31.5-59.6,43.6-46.8,8.6,9.1-2.4,22.2-17.7,20.7C82,119.8,71.9,104.5,75.3,88c6.7-32.7,58.3-54.4,60.5-55.9"
        fill="none"
        strokeWidth="3"
        strokeDasharray="6 7"
        stroke="url(#linear-gradient)"
      ></path>
      <path
        d="M141,21.5s-.5-12.3-3.7-15.6a8.4,8.4,0,0,0-11.8,11.8C128.7,21,141,21.5,141,21.5Z"
        fill="#ced8e5"
        opacity="0.4"
      ></path>
      <path
        d="M146.7,26.4s.5,12.3,3.7,15.6a8.4,8.4,0,0,0,11.8-11.8C159.1,26.9,146.7,26.4,146.7,26.4Z"
        fill="#ced8e5"
        opacity="0.4"
      ></path>
      <path
        d="M137,24a8.1,8.1,0,0,1,8.1-8.1h6.3v6.3a8.1,8.1,0,0,1-8.1,8.1H137Z"
        fill="#a8b7c5"
      ></path>
      <g mask="url(#mask)">
        <rect
          x="140.1"
          y="16.7"
          width="2.3"
          height="16.69"
          transform="translate(23.6 107.2) rotate(-45)"
          fill="#dde5f2"
        ></rect>
        <path d="M139.1,16.2l1.6-1.6,11.8,11.8L150.9,28Z" fill="#dde5f2"></path>
      </g>
      <path
        d="M159.2,12.3l5.5-1.3"
        fill="none"
        stroke="#8696c5"
        strokeWidth="0.9"
      ></path>
      <path
        d="M156.9,10.1l2.1-5.5"
        fill="none"
        stroke="#8696c5"
        strokeWidth="0.9"
      ></path>
      <path
        d="M153.7,20.5a6.1,6.1,0,1,0-6.1-6.1A6.1,6.1,0,0,0,153.7,20.5Z"
        fill="#8b9aa8"
      ></path>
      <path d="M24.1,105.5,110,132.8V229L24.1,192Z" fill="#dae2ec"></path>
      <path d="M180.5,109.2l-71.7,23.6V229l71.7-37Z" fill="#c5d0d9"></path>
      <path
        d="M180.3,109.1,208,146.2l-73.6,29L109,132.6Z"
        fill="#dae2ec"
      ></path>
      <path d="M24,105.3,0,142.8l84.6,32.4,24.7-42.7Z" fill="#c5d0d9"></path>
    </svg>
  </svg>
);

export const Close = () => (
  <svg
    className="svg-next-icon ml-2 svg-next-icon-size-10"
    width="10"
    height="10"
  >
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
      <path d="M18.263 16l10.07-10.07c.625-.625.625-1.636 0-2.26s-1.638-.627-2.263 0L16 13.737 5.933 3.667c-.626-.624-1.637-.624-2.262 0s-.624 1.64 0 2.264L13.74 16 3.67 26.07c-.626.625-.626 1.636 0 2.26.312.313.722.47 1.13.47s.82-.157 1.132-.47l10.07-10.068 10.068 10.07c.312.31.722.468 1.13.468s.82-.157 1.132-.47c.626-.625.626-1.636 0-2.26L18.262 16z"></path>
    </svg>
  </svg>
);

export const Verify = () => (
  <svg
    className="svg-next-icon mr-3 svg-next-icon-size-16"
    width="16"
    height="16"
  >
    <svg viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        fill-rule="evenodd"
        clip-rule="evenodd"
        d="M1.41663 8.50002C1.41663 4.59002 4.58996 1.41669 8.49996 1.41669C12.41 1.41669 15.5833 4.59002 15.5833 8.50002C15.5833 12.41 12.41 15.5834 8.49996 15.5834C4.58996 15.5834 1.41663 12.41 1.41663 8.50002ZM7.08329 10.0371L11.7512 5.36918L12.75 6.37502L7.08329 12.0417L4.24996 9.20835L5.24871 8.2096L7.08329 10.0371Z"
        fill="#2962FF"
      ></path>
    </svg>
  </svg>
);

export const Warning = () => (
  <svg
    width="14px"
    height="14px"
    viewBox="0 0 50 47"
    style={{ fill: "rgb(253, 189, 65)" }}
  >
    <path d="M28.5714 38.5078C28.5714 39.01 28.1808 39.4286 27.6786 39.4286H22.3214C21.8192 39.4286 21.4286 39.01 21.4286 38.5078V33.2065C21.4286 32.7042 21.8192 32.2857 22.3214 32.2857H27.6786C28.1808 32.2857 28.5714 32.7042 28.5714 33.2065V38.5078ZM28.5156 28.0725C28.4877 28.4353 28.0692 28.7143 27.567 28.7143H22.4051C21.875 28.7143 21.4565 28.4353 21.4565 28.0725L20.9821 15.3214C20.9821 15.154 21.0658 14.875 21.2612 14.7355C21.4286 14.596 21.6797 14.4286 21.9308 14.4286H28.0692C28.3203 14.4286 28.5714 14.596 28.7388 14.7355C28.9342 14.875 29.0179 15.0982 29.0179 15.2656L28.5156 28.0725ZM28.125 2.01228C27.5112 0.868302 26.3114 0.142854 25 0.142854C23.6886 0.142854 22.4888 0.868302 21.875 2.01228L0.446429 41.298C-0.167411 42.3862 -0.139509 43.7254 0.502232 44.8136C1.14397 45.9018 2.31585 46.5714 3.57143 46.5714H46.4286C47.6842 46.5714 48.856 45.9018 49.4978 44.8136C50.1395 43.7254 50.1674 42.3862 49.5536 41.298L28.125 2.01228Z"></path>
  </svg>
);

export const NoData = () => (
  <svg
    className="svg-next-icon mb-3 svg-next-icon-size-100"
    width="100"
    height="100"
  >
    <svg
      id="Layer_1"
      data-name="Layer 1"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 208 230"
    >
      <defs>
        <linearGradient
          id="linear-gradient"
          x1="133.69"
          y1="120.61"
          x2="73.06"
          y2="91.22"
          gradientTransform="matrix(1, 0, 0, -1, 0, 231.28)"
          gradientUnits="userSpaceOnUse"
        >
          <stop offset="0" stopColor="#95abc2"></stop>
          <stop offset="1" stopColor="#fff"></stop>
        </linearGradient>
        <mask
          id="mask"
          x="134.5"
          y="14.55"
          width="17.97"
          height="17.21"
          maskUnits="userSpaceOnUse"
        >
          <g id="mask0">
            <path
              d="M137,24a8.1,8.1,0,0,1,8.1-8.1h6.3v6.3a8.1,8.1,0,0,1-8.1,8.1H137Z"
              fill="#fff"
            ></path>
          </g>
        </mask>
      </defs>
      <title>Artwork</title>
      <path d="M94.2,85.7l85.9,23.6v82.6l-85.9-37Z" fill="#8e99a3"></path>
      <path d="M94.3,85.7,24.1,105.3v86.8l70.1-37.6Z" fill="#a0aab5"></path>
      <path
        d="M139.4,185.9c-19.4-5.7-53.4-5.2-66.2-38.3-11.7-30.4,31.5-59.6,43.6-46.8,8.6,9.1-2.4,22.2-17.7,20.7C82,119.8,71.9,104.5,75.3,88c6.7-32.7,58.3-54.4,60.5-55.9"
        fill="none"
        strokeWidth="3"
        strokeDasharray="6 7"
        stroke="url(#linear-gradient)"
      ></path>
      <path
        d="M141,21.5s-.5-12.3-3.7-15.6a8.4,8.4,0,0,0-11.8,11.8C128.7,21,141,21.5,141,21.5Z"
        fill="#ced8e5"
        opacity="0.4"
      ></path>
      <path
        d="M146.7,26.4s.5,12.3,3.7,15.6a8.4,8.4,0,0,0,11.8-11.8C159.1,26.9,146.7,26.4,146.7,26.4Z"
        fill="#ced8e5"
        opacity="0.4"
      ></path>
      <path
        d="M137,24a8.1,8.1,0,0,1,8.1-8.1h6.3v6.3a8.1,8.1,0,0,1-8.1,8.1H137Z"
        fill="#a8b7c5"
      ></path>
      <g mask="url(#mask)">
        <rect
          x="140.1"
          y="16.7"
          width="2.3"
          height="16.69"
          transform="translate(23.6 107.2) rotate(-45)"
          fill="#dde5f2"
        ></rect>
        <path d="M139.1,16.2l1.6-1.6,11.8,11.8L150.9,28Z" fill="#dde5f2"></path>
      </g>
      <path
        d="M159.2,12.3l5.5-1.3"
        fill="none"
        stroke="#8696c5"
        strokeWidth="0.9"
      ></path>
      <path
        d="M156.9,10.1l2.1-5.5"
        fill="none"
        stroke="#8696c5"
        strokeWidth="0.9"
      ></path>
      <path
        d="M153.7,20.5a6.1,6.1,0,1,0-6.1-6.1A6.1,6.1,0,0,0,153.7,20.5Z"
        fill="#8b9aa8"
      ></path>
      <path d="M24.1,105.5,110,132.8V229L24.1,192Z" fill="#dae2ec"></path>
      <path d="M180.5,109.2l-71.7,23.6V229l71.7-37Z" fill="#c5d0d9"></path>
      <path
        d="M180.3,109.1,208,146.2l-73.6,29L109,132.6Z"
        fill="#dae2ec"
      ></path>
      <path d="M24,105.3,0,142.8l84.6,32.4,24.7-42.7Z" fill="#c5d0d9"></path>
    </svg>
  </svg>
);
